...
def main():
    ...
    for page_idx in range(1, num_pages + 1):
        ...
        div_elements = soup.find_all("div", class_="VwiC3b yXK7lf MUxGbd yDYNvb lyLwlc lEBKkf")

        # Extract emails from <span> tags within the specified <div> elements
        emails_from_div = []
        for div_element in div_elements:
            span_tags = div_element.find_all("span")
            emails_from_span = [tag.get_text() for tag in span_tags if re.match(r"[a-zA-Z0-9._%+-]{2,}@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}", tag.get_text())]
            emails_from_div.extend(emails_from_span)

        # Combine all extracted emails and remove duplicates
        all_emails = set(emails_from_div)
